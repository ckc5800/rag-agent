...
- rag-agent — LangGraph Corrective-RAG + 하이브리드 검색(FAISS·BM25 RRF). 평가셋을 10 → 140문항으로 늘려가며 검색/생성 분리 측정, 코퍼스 15.6배 확장 후에도 튜닝값 유지 확인 (140문항 80.7·81.4%, 2회). 채점기 결함을 찾아내 6%p 보정
...
- portfolio-mcp — 포트폴리오 조회용 MCP 서버·클라이언트 양쪽 구현. read-only 도구 10종(프로필·프로젝트 목록/상세·논문·BM25 검색·타임라인·기술 경험 확인·실시간 조회) + 리소스 6개 + 프롬프트 3개, 의존성 2개. 도구 조합 평가 20문항으로 회귀 감시
- pdm-agent — 센서 이상탐지 + LLM 진단 파이프라인. NASA C-MAPSS 4개 셋에서 자명한 대조군과 비교 — 경보 유효율 93.6~98.0%(오탐 0~0.9%, knee 125 가정), 공식 test 셋 RUL MAE 15.8~23.0사이클. 비용축 운전점 선택(경보 없음 대비 90% 절감)과 위험-커버리지 곡선까지 측정, 뒤집힌 결론 7건 기록
