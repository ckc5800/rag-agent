...
def main() -> int:
    ...
    # 환경 지문은 **조건 루프가 config를 변형하기 전에** 찍는다 — 나중에
    # 찍으면 마지막 조건의 값이 기록돼 실행 조건을 오해하게 된다.
    env = run_metadata()
    print(f"{len(cases)}문항 × {len(args.values)}조건 × {args.repeat}회 "
          f"(model={env['llm_model']}, GENERATE_TOP_N={config.GENERATE_TOP_N})\n")
    ...
    RESULTS.write_text(json.dumps(
        {"env": env, "repeat": args.repeat,
         "generate_top_n": env["params"]["GENERATE_TOP_N"],
