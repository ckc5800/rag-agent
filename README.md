...
| | 표 청킹 | +한자 금지 한 줄 | +재생성 가드 |
|---|---|---|---|
| 전체 | 86 / 86 | **89 / 89** | 88 / 88 |
| 한자 섞인 답변 | 10 | 6 | **2** |
| comparison | 12/15 | 14/15 | 14/15 |
| trap | 7/10 | 8/10 | 8/10 |
| 지연 | 1.8~2.1s | 1.6s | 1.7s |
...
#### 가드가 정확도를 1%p 깎은 것처럼 보이는 이유

남은 6건은 전부 채점을 통과한다 — **정확도로는 안 보이는 결함**이라 가드를
붙였다(한자 감지 → 온도 0.5로 1회 재생성, 재시도도 한자면 원본 유지). 결과는
한자 6 → 2, 지연 +0.1s, 그런데 정확도 89 → 88.

깎인 한 문항을 열어 보니 가드 탓이 아니었다. "손실 함수 조합은 몇 가지"의 정답은
5인데, 지시만 준 답변은 **한국어 부분에서 개수를 말하지 않고 중국어 문장에서만**
`总共比较了5种`이라고 맞혔다. 채점 패턴이 중국어 단위어(`5种`)를 허용하고 있어
통과한 것이다. 가드가 그 문장을 한국어로 바꾸자 같은 모델이 5개를 나열해 놓고
"4가지"라고 셌다.

**중국어 이탈이 한국어 쪽 계산 실패를 가리고 있었다.** 중국어 단위어를 정답으로
인정하지 않고 다시 세면 네 런이 전부 88이다 — 가드는 정확도 중립이고 한자만
6 → 2로 줄인다. 그래서 채택한다. 남은 2건은 재시도까지 이탈한 경우다.

이 문항의 `5种` 허용은 채점기 확장(§10)에서 "qwen 혼입의 새 변형"을 잡으려고
넣은 것인데, **한국어로 답해야 하는 시스템에서 중국어 단위어를 정답으로
인정하는 것**이라 방향이 반대다. 지금은 기록만 남기고 패턴은 그대로 둔다 —
바꾸면 위 표의 숫자가 과거 측정과 비교 불가능해진다.
