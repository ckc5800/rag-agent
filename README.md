...
| | 표 청킹 | +한자 금지 한 줄 | +고쳐쓰기 | +문장 필터 |
|---|---|---|---|---|
| 전체 | 86 / 86 | **89 / 89** | 89 / 87 | 86 / 87 |
| 한자 섞인 답변 | 10 | 6 | 1~2 | **0 / 0** |
| comparison | 12/15 | 14/15 | 14/15 | 14/15 |
| trap | 7/10 | 8/10 | 8/10 | 8/10 |
| 지연 | 1.8~2.1s | 1.6s | 1.7s | 1.7s |
...
#### 그런데 교정 출력도 이탈한다 — 마지막 겹은 LLM 없이

고쳐쓰기로 한자 답변이 1~2건까지 줄었지만 0이 되지 않았다. 남은 것을 열어 보니
**교정 호출이 같은 자리에서 또 넘어간다**(`데이터를清理和预处理…`). LLM을
거치는 수단은 전부 같은 벽에 부딪힌다.

그래서 마지막 겹은 결정적이다 — 중국어로 넘어간 문장만 버리고 한국어 문장을
남긴다. 문장 하나에 한자가 3자 이상이면 이탈로 본다(이탈은 문장을 통째로
중국어로 쓰므로 수십 자가 나오고, 코퍼스의 정당한 인용 `古語`는 두 자다).
건질 한국어 문장이 없으면 원본을 유지한다 — 첫 문장 중간에서 샌 경우다.

세 겹으로 **한자 답변 0/0**이 됐다.

#### 값: 2~3%p

공짜가 아니다. 89% → 86·87%로 내려간다. 내려간 세 문항을 열어 보면 성격이 다르다.

| 문항 | 무슨 일 |
|---|---|
| 몇 개 언어 | 가드 후 "32**가지** 언어" — 정답인데 채점 패턴이 `32\s*(개\|개국)`이라 탈락 |
| TTFB | 교정이 답을 요약하면서 `2292`·`334`를 흘렸다 (실제 손해) |
| 손실 함수 조합 | 개수(5)를 **중국어 문장만** 들고 있었다 — 그 문장을 버리니 근거가 사라짐 |

셋 중 하나는 채점기 경직성이고, 하나는 교정의 손실이고, 하나는 **원래부터
한국어 답변이 불완전했다는 사실이 드러난 것**이다. 마지막이 이 실험의 결론과
같다 — 중국어 이탈이 한국어 쪽 실패를 가리고 있었다.

이 프로젝트는 포트폴리오 데모다. 채점 지표 2%p보다 "한국어 안내가 중국어로
답한다"가 먼저 보이는 결함이라 세 겹을 다 켠다. 숫자를 되돌리려면 문장 필터
한 겹만 빼면 된다(한자 1~2건, 89%).

`5种`을 정답으로 인정하던 채점 패턴(§10)도 이 실험이 드러낸 문제다 —
한국어로 답해야 하는 시스템에서 중국어 단위어를 정답으로 세고 있었다. 지금은
기록만 남기고 패턴은 그대로 둔다. 바꾸면 위 표가 과거 측정과 비교 불가능해진다.
