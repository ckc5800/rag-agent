...
├── loaders.py    비마크다운 코퍼스 포맷 — PDF/DOCX 텍스트 추출 (아래 참고)
...
## 비정형 문서 확장 — PDF·DOCX 파싱

채용 공고(그래파이 등)가 요구한 "비정형 데이터(PDF/DOCX/HWP) 처리"를 실제
코퍼스 확장으로 구현했다. `loaders.py`가 PDF(`pypdf`)·DOCX(`python-docx`)를
텍스트로 뽑아 마크다운과 같은 파이프라인(청킹→임베딩→인덱싱)에 합류시킨다.
**HWP는 만들지 않았다** — 이 PC·계정 전체를 검색해도 실물 `.hwp` 파일이
0건이라(2026-08 확인), 검증할 실물 없이 파서를 짜면 동작을 아무도 확인할
수 없다. 이 프로젝트가 시작부터 지켜온 "실물로만 검증한다" 원칙을 여기서도
따른다.

**실제 문서 2건으로 파서를 검증**했다(합성 테스트 문서가 아니다) —
`segmentation-paper.pdf`(3D 시맨틱 세그멘테이션 논문 발표자료, 21p)와
`resume-original.docx`(검증된 정본 이력서, 2026-08-01 근거 감사를 거친 버전).
...
테스트(`tests/test_loaders.py`)는 합성 문서가 아니라 실제 PDF·DOCX로
