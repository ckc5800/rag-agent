...
#
# **실측 완료(eval/ab_rerank.py, 55문항 × 2회, 2026-08)** — 도입 후 한 번도
# 안 쟀던 것을 처음 쟀다. 결과는 완전한 동률이다:
#     RERANK=off  78% (86/110) · 지연 중앙값 2.6s
#     RERANK=on   78% (86/110) · 지연 중앙값 5.1s
# 뒤집힌 문항 8개도 상승 4 / 하락 4로 방향성이 없다(노이즈). **정확도는
# 그대로인데 지연만 2배**라 켤 이유가 없다 — 기본 꺼둠을 유지하되, 이제는
# "안 재봐서"가 아니라 "재봤더니 값을 못 해서"다.
