...
# 아직 A/B 전이라 기본은 꺼둔다.
# **실측 완료(eval/eval_verify.py, 7b 답변 75건, 2026-08-11): 기본 꺼둠 유지.**
# 실질 오경보 1/54(2%) · 실질 오답 검출 2/6(33%) · 파싱 실패 0 · +1.4s/질의.
# 정밀도는 좋으나 재현율이 낮아 상수 심판("항상 grounded", 일치 54/60=90%)
# 대비 +2검출/−1오경보 — 정답률 85~90% 평가셋에서는 심판이 얹을 정보가
# 애초에 적다. 켠다면 거부 답변 skip 가드가 필수다: 거부에는 지어낸 주장이
# 없어 grounded 의미가 정의되지 않는데, 심판이 정답-거부의 7/10을 "문서에
# 없다"는 맞는 말과 함께 UNGROUNDED로 냈다(측정에서 이 버킷을 안 갈랐다면
# "오경보 12.5%"로 완전히 잘못 읽혔을 결함 — eval_verify.py 주석 참고).
