...
#
# A/B 완료(7B, 51문항×2회): RERANK=1 이 85% 로 기본(85.3%)과 동일하고
# 지연만 1.7s → 2.9~3.7s(약 2배)로 늘었다. **끄는 것이 맞다.**
# 단 이유는 "리랭커가 나빠서"가 아니라 **재정렬할 대상이 없어서**다 —
# 이 코퍼스는 satisfied@6 이 97~100% 라 상위 6개 안에 이미 답이 다 들어와
# 있다. 후보가 수백 개가 되는 규모에서는 결론이 달라질 수 있다.
