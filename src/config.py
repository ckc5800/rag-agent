...
#
# **7b·100문항 재검증(2026-08-12, agg·enum 26문항×3회): 채택 근거가
# 확정으로 올라섰다.** 기존 근거는 3b 시절 "손해 근거 없음" 수준이었다.
#     변수(실제 라우팅 19문항)  34/57 → 45/57   +11판정 (60% → 79%)
#     대조군(폴백 7문항, 동일)  15/21 → 15/21   ±0 — **노이즈 0**
# 대조군이 완전히 평평한 첫 실행이라 +19%p가 온전히 효과다. 구제 5문항
# (배포 자동화·모니터링 열거 0/3→3/3, 학회별 집계 3건) vs 악화 1문항
# (한국자동차공학회 집계 3/3→0/3 — aggregation 오버라이드의 TOP_N=3이
...
# 다시 켜고 재측정할 것.
# **7b 재측정(2026-08-11, 26문항×2회): 7b에서도 켜지 않는다 — 오히려 손해.**
# single 33/52(63%) vs routed 29/52(56%) vs all_team 22/52(42%).
# routed가 single과 갈리는 comparison 10문항에서 14/20 → 10/20, 깨진 3문항
# (논문vs특허 등)이 모두 양쪽 반복에서 재현 — 전부 "두 사실이 한 청크에
# 요약돼 있는" 문서화된 실패 패턴이다. 모델 크기별 지도: **3b ≈ 0 /
# 7b −8%p / 14b +18%p** — 단조가 아니다. 7b쯤엔 단일 RAG의 비교 추론이
# 충분히 좋아져 분해의 이득은 줄고 실패 모드만 남고, 14b는 planner·
# synthesizer가 그 손해를 뒤집을 만큼 강하다. 14b를 기본으로 쓸 때만 켤 것.
...
# 아직 A/B 전이라 기본은 꺼둔다.
# **실측 완료(eval/eval_verify.py, 2026-08-11~12): 기본 꺼둠 유지.**
# 세 번의 독립 측정(75문항 + 100문항 run1·run2, 합산 200판정)이 같은 그림:
#     실질 오경보  1/54 · 3/73 · 2/71   → 합산 5/144 (3.5%)
#     실질 오답 검출 2/6 · 3/9 · 4/11   → 합산 7/20 (35%)
#     파싱 실패 0 · 판정 지연 +1.4~2.5s/질의
# 정밀도는 높고(~96%) 재현율이 낮은(35%) 심판 — 상수 심판("항상 grounded")
# 대비 순증 +7검출/−5오경보 = +2/200이라 게이트용으론 비용(+LLM 1회)을
# 정당화하지 못한다. 켠다면 거부 답변 skip 가드가 필수다: 거부에는 지어낸
# 주장이 없어 grounded 의미가 정의되지 않는데, 심판이 정답-거부의 70%대를
# "문서에 없다"는 맞는 말과 함께 UNGROUNDED로 낸다(이 버킷을 안 갈랐다면
# 오경보가 12.5%로 과대 계상됐을 측정기 결함 — eval_verify.py 주석 참고).
